...
Proccesses associated with finding, opening, or reading
files. 
"""
...
def file_signature(fpath):
    ...
    to more than one (see :ref:`reference-label`).
    If no matching file signatures found, returns "unknown".
    
    Requirements: a YAML file with file formats and signatures to run.
    The YAML should be called `fileformat.yml` and saved in the `utilsx` pacakage.
    
    `fileformat.yml` structure:
    ...
    by referring to the `File Signatures website <https://filesignatures.net/index.php?search=json&mode=EXT>`_.
    ...
    References
    ------------
    .. [1] https://filesignatures.net/index.php?search=json&mode=EXT
    
    .. [2] https://www.garykessler.net/library/file_sigs.html
    
    .. [3] https://hackernoon.com/determining-file-format-using-python-c4e7b18d4fc4
    """
...
def get_absolute_path(url, doc_url):
    ...
    Relative URL (Location following the root domain):
...
def read_yaml(params, abort=True):
    ...
    ---------
    dict
        Data block read from yaml stored in a dictionary.
    ...
    * abort = False: continues running but will return False instead of a data block.
    ...
    If abort set to False, user can error check returned value by verifying if False.
    """
...
def scan_csv_list(url):
    ...
    References
    ------------
    .. [1] https://stackoverflow.com/questions/60924860/python-get-list-of-csv-files-in-public-github-repository
    """
...
def unicodify(seq, min_confidence=0.5):
    ...
    References 
    ------------
    .. [1] https://stackoverflow.com/questions/14856872/check-encoding-and-convert-to-unicode
    """
